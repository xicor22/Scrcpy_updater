...
def download():
    ...
    final = 'S:\\Tools\\Scrcpy\\scrcpy.zip'
    wget.download(end_link, final)
    return final


def extract():
    with zp('S:\\Tools\\Scrcpy\\scrcpy.zip', 'r') as zipObj:
        zipObj.extractall('S:\\Tools\\Scrcpy')


def folder_loc():
    process = subprocess.Popen(
        ["powershell", "cd S:\\Tools\\Scrcpy\\ ; ls"], stdout=subprocess.PIPE)
...
def move(folder_name):
    des = 'S:\\Tools\\Scrcpy\\'
    fol_loc = 'S:\\Tools\\Scrcpy\\'+folder_name
